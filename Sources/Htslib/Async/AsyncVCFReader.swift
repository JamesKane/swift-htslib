...
/// Actor-isolated asynchronous reader for VCF/BCF files.
///
...
/// Usage:
/// ```swift
/// let reader = try AsyncVCFReader(path: "file.vcf.gz", loadIndex: true)
/// // Sequential reading
...
/// while let record = try await reader.next() { ... }
/// ```
public actor AsyncVCFReader {

    // MARK: - Storage

    private nonisolated(unsafe) let filePointer: UnsafeMutablePointer<htsFile>

    /// The VCF header — accessible without `await`.
    public nonisolated let header: VCFHeader

    /// The file path — accessible without `await`.
    public nonisolated let path: String

    // Record buffer for reading
    private nonisolated(unsafe) var record: UnsafeMutablePointer<bcf1_t>?
    ...
    /// Open a VCF/BCF file for async reading.
    ///
    /// - Parameters:
    ...
    public init(path: String, loadIndex: Bool = false) throws {
        ...
        guard let hdr = bcf_hdr_read(fp) else {
            hts_close(fp)
            throw HTSError.headerReadFailed
        }
        self.header = VCFHeader(pointer: hdr)

        self.record = bcf_init()

        if loadIndex {
            guard let idx = path.withCString({ hts_idx_load3($0, nil, 0, 0) }) else {
    ...
    /// Open a VCF/BCF file for async reading with a dedicated thread pool.
    ///
    /// - Parameters:
    ///   - path: Path to the file.
    ...
    ///
    /// - Returns: The next `VCFRecord`, or `nil` at end-of-file/region.
    /// - Throws: `HTSError.readFailed` on I/O error.
    public func next() throws -> VCFRecord? {
        guard !exhausted, let rec = record else { return nil }

        let ret: Int32
        if inQuery, let iter = queryIterator {
            ret = hts_shim_bcf_itr_next(filePointer, iter, rec)
        } else {
            ret = bcf_read(filePointer, header.pointer, rec)
        }
    ...
    public func query(region: String) throws {
        ...
        guard let iter = hts_shim_bcf_itr_querys(idx, header.pointer, region) else {
    ...
    public func query(tid: Int32, start: Int64, end: Int64) throws {
        ...
        guard let iter = hts_shim_bcf_itr_queryi(idx, tid, start, end) else {
